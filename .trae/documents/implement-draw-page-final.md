...
**Architecture:** DrawPage 采用左右分栏（左配置 360px / 右结果）。配置面板通过 `useEventStore` 加载赛事/轮次/队伍，组装 `DrawParams` 后调用 `useDrawStore.execute()`。结果区有标准列表模式与大屏全屏覆盖模式。抽取动画用纯 CSS keyframes 实现翻牌/旋转效果。多选筛选通过扩展 `TopicFilter` 数组字段 + `buildWhereClause` 的 `IN(?,?)` 实现，FilterPanel 全局升级。
...
### 已实现
- **后端**：`draw-engine.ts` 抽取引擎、`probability.ts` 概率控制、`draw.ipc.ts` 6 个 IPC 通道、`draw.repo.ts` 数据访问层 — 全部完成
- **前端 Store**：`drawStore.ts`（execute/redo/listSessions）、`eventStore.ts`（events/rounds/teams CRUD）、`topicStore.ts` — 全部完成
- **Preload 桥接**：`window.drawAPI.*` 6 个方法已暴露 — 完成
- **基础组件**：`FilterPanel.tsx`（单选版）、`TopicCard.tsx` — 已存在
- **App 路由**：`/draw` 已注册为默认首页 — 完成

### 未实现（本计划要做的）
- `DrawPage.tsx` 当前是占位（13 行 Result 组件），需完全重写
- `src/renderer/src/components/draw/` 子目录及 6 个子组件不存在
- `src/renderer/src/styles/draw.css` 不存在
- `TopicFilter` 多选数组字段（types/domains/difficulties）未实现
- `buildWhereClause` 不识别数组字段
- `FilterPanel` 的 type/domain/difficulty 仍是单选
- `main.tsx` 未引入 `draw.css`

### 设计文档关键要求（5.1-5.3 节）
- **5.1 抽取流程**：构建候选池 → 排除黑名单/已抽/历史 → 加权随机 → 随机分配正反方 → 落库 + 审计
- **5.2 权重与难度**：weight 默认 1.0；轮次 `difficulty_override` 决定难度分布；题源混合比例（如 7:3）
- **5.3 大屏展示**：投屏模式按钮 → 全屏深色背景 → 逐题揭晓（点击"下一题"）→ 显示辩题/对阵/轮次 → ESC 退出
...
### 新增文件（7 个）
- `src/renderer/src/components/draw/DrawConfigPanel.tsx` — 左侧抽取配置面板
- `src/renderer/src/components/draw/TeamPairing.tsx` — 队伍对阵配对编辑器
- `src/renderer/src/components/draw/DrawResultCard.tsx` — 单条抽取结果卡片
...
- `src/renderer/src/styles/draw.css` — 翻牌/淡入/缩放/大屏渐变 keyframes

### 修改文件（5 个）
- `src/shared/types.ts` — `TopicFilter` 增加 `types?/domains?/difficulties?` 数组字段
- `src/main/db/repository/topic.repo.ts` — `TopicFilter` 同步 + `buildWhereClause` 处理数组生成 `IN (?,?,?)`
...
1. **多选筛选**：扩展 `TopicFilter` 增加 `types/domains/difficulties` 数组字段，保留单值字段向后兼容。`buildWhereClause` 数组优先（若数组非空，单值字段被忽略）。FilterPanel 全局升级 type/domain/difficulty 为多选，source/source_type/status 保持单选。
2. **动画**：纯 CSS keyframes（翻牌、淡入、旋转、脉冲、大屏渐变），不引入 framer-motion（项目依赖中无动画库）。
3. **大屏模式**：fixed 定位全屏覆盖层（z-index 9999），ESC 退出，左右箭头切换。不用路由（避免新窗口复杂度）。
4. **抽取动画**：点击抽取后显示至少 1.2s 旋转加载遮罩，与 IPC 调用 `Promise.all` 同步。
5. **关键词包含/排除**：v1 简化，DrawConfigPanel 中的 includeKeywords/excludeKeywords **不传后端**（draw-engine 服务端抽取，TopicFilter 不支持排除词，且 buildCandidatePool 已强制 status='active'）。UI 保留输入框但仅作记录。**FilterPanel 中的 keyword 单值字段仍传后端。**
6. **队伍配对语义**：用户在 UI 层手动或随机配对 `TeamPair[]`，但 `draw-engine.assignStances` 内部会 Fisher-Yates 洗牌后两两配对（用户配对顺序会被引擎打乱）。`buildParams()` 把 `teamPairs` 扁平化为 `teams: Team[]` 传给 `DrawParams.teams`。UI 上的"对阵配对"主要起**数量校验**作用（确保偶数队伍）。
7. **operator**：v1 固定为 `'renderer'`，后续接用户系统时扩展。
8. **题源混合比例**：滑块 0-100，对应 officialRatio 0%-100%，传给 draw-engine 的 `source_mix_ratio`。默认关闭，开启后默认 70（7:3）。
9. **BigScreen 修复**：原计划的 `bigscreen-reveal-hidden/shown` 类名逻辑有 bug（`revealedCount > 0` 永真），本计划修正为：当前展示的题目始终清晰，无需切换 hidden/shown 类。底部按钮区去除重复的"下一题"按钮。
10. **测试基线**：现有 67 个单元测试需全部通过，无回归。
...
- Modify: `src/main/db/repository/topic.repo.ts` (TopicFilter 接口同步)

**说明：** 在两侧 `TopicFilter` 接口中新增 3 个数组字段（`types/domains/difficulties`），保留原单值字段向后兼容。

- [ ] **Step 1: 修改 `src/shared/types.ts` 的 TopicFilter 接口**

在 `TopicFilter` 接口末尾追加 3 个数组字段：
...
- [ ] **Step 2: 同步修改 `src/main/db/repository/topic.repo.ts` 顶部的 TopicFilter 接口**

追加相同 3 个数组字段，保持两侧类型一致。
...
**说明：** 在 `buildWhereClause` 中处理 `types/domains/difficulties` 数组，生成 `column IN (?, ?, ?)`。数组优先于单值字段（若同时存在，数组生效，单值字段被忽略）。

- [ ] **Step 1: 修改 buildWhereClause 函数**

将原 `scalarFields` 循环改为先处理数组字段、再处理单值字段（单值字段需跳过对应数组已设置的情况）。完整替换 `buildWhereClause` 函数体：
...
Expected: typecheck 通过；67 个测试无回归
...
将"类型/领域/难度"三个 Field 的 Select 替换为：
...
- Modify: `src/renderer/src/main.tsx` (引入 CSS)

**说明：** 纯 CSS keyframes 实现翻牌、淡入、缩放、大屏背景渐变。无 JS 依赖。

- [ ] **Step 1: 创建 `src/renderer/src/styles/draw.css`**
...
- [ ] **Step 2: 在 `src/renderer/src/main.tsx` 顶部 import 区追加**

在 `import './index.css';` 之后追加：
...
**说明：** 当 `include_stance=true` 时显示。用户从赛事队伍中两两配对成 A vs B。要求队伍数为偶数。UI 用列表展示配对，支持"自动随机配对"和"手动调整"。**注意：持方（正/反）由 draw-engine 在抽取时随机分配，此处仅配置对阵双方。**
...
**说明：** 全屏覆盖层。深色渐变背景。逐题揭晓：默认显示"准备抽取"占位，点击"下一题"逐张揭示（每次只显示当前一题，已揭晓的不显示）。ESC 退出。显示辩题大字、正反方对阵、轮次。支持左右箭头键切换。**底部按钮区只保留 [上一题] + [下一题/开始揭晓/全部揭晓退出] 两个按钮，避免重复。**
    ...
    <div className="bigscreen-overlay">
      ...
      >
        {!currentTopic ? (
          ...
          <div className="bigscreen-reveal" style={{ textAlign: 'center', width: '100%' }}>
      ...
      <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
        ...
        {!allRevealed ? (
          <Button
            ...
            style={{ minWidth: 200, height: 56, fontSize: 18 }}
...
**说明：** 点击"开始抽取"后短暂显示的翻牌动画覆盖层（约 1.2 秒）。显示旋转加载图标 + "正在抽取..."文字。动画结束后自动消失（由父组件控制 `open` 状态）。
...
**说明：** 编排所有子组件。管理本地状态（配置、动画、大屏）。组装 `DrawParams` 调用 `useDrawStore.execute()`。

- [ ] **Step 1: 重写 DrawPage.tsx**

```typescript
import { useEffect, useState, useMemo } from 'react';
import { Layout, Empty, message, Typography, theme } from 'antd';
import { ThunderboltOutlined } from '@ant-design/icons';
...
- [x] Task 1-2 覆盖多选筛选后端（types/domains/difficulties 数组字段 + IN 子句）
- [x] Task 3 覆盖 FilterPanel 全局升级（type/domain/difficulty 改多选）
- [x] Task 4 覆盖动画 CSS（翻牌/淡入/旋转/脉冲/大屏渐变）
- [x] Task 5-6 覆盖配置面板（含队伍对阵 + 题源比例 + 筛选条件）
- [x] Task 7-8 覆盖标准结果展示（卡片列表 + 顶部操作栏）
- [x] Task 9 覆盖大屏模式（全屏 + 逐题揭晓 + ESC 退出 + 修复按钮重复 bug）
- [x] Task 10 覆盖抽取动画（1.2s 旋转遮罩）
- [x] Task 11 编排主页面（左右分栏 + 状态管理 + IPC 调用）
- [x] Task 12 验证（typecheck + 67 测试 + dev 启动）
- [x] 无占位符（所有代码块完整）
- [x] 类型一致（DrawParams/DrawResult/TopicFilter 跨任务一致）
- [x] 修复了原计划 BigScreen 中 `bigscreen-reveal-hidden/shown` 类名逻辑 bug（改为始终 `bigscreen-reveal`）
- [x] 修复了原计划 BigScreen 底部按钮重复（去除多余的右侧"下一题"）
- [x] 修复了原计划 DrawConfigPanel 切换赛事时未重置 teamPairs 的 bug（在 onChange 中直接重置）
- [x] 符合用户规则（所有文件在 f:\ 盘，不污染 C 盘）
