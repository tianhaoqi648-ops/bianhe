 ...
 *   3. 从关联表回填现有记录的快照值
 *
 * 防御性迁移：旧表可能缺少 stage_remaining_cache / aff_remaining_ms / neg_remaining_ms
 * 列（即使相关迁移标记为已应用，ALTER TABLE 也可能因各种原因实际未生效）。
 * 动态检查旧表列，缺失的列在 SELECT 中用 NULL 替代，避免"no such column"错误。
 */
