...
describe('judge_speech：写评审历史', () => {
  ...
  it('写入历史含 provenance 字段（provider/model/mode=stage/inputHash）', async () => {
    mockChat.mockResolvedValue({ role: 'assistant', content: VALID_JSON })
    const res = await judgeSpeechTool.execute(VALID_ARGS, ctxWithConfig)
    expect(res.success).toBe(true)
    const input = mockJudgeHistoryCreate.mock.calls[0][0]
    ...
    expect(input.provenance.mode).toBe('stage')
    expect(input.provenance.inputHash).toMatch(/^[0-9a-f]{8}$/)
    // 同输入 → 确定性 input hash
    await judgeSpeechTool.execute(VALID_ARGS, ctxWithConfig)
